			...
			if extra, hasExtra := updateData["@extra"].(string); hasExtra {
				...
				if msgType, hasType := updateData["@type"]; hasType {
					...
					if client.rawUpdates != nil {
						...
						client.rawUpdates <- tdlib.UpdateMsg{Data: updateData, Raw: updateBytes}
					}
...
func (client *Client) GetRawUpdatesChannel(capacity int) chan tdlib.UpdateMsg {
	client.rawUpdates = make(chan tdlib.UpdateMsg, capacity)
...
func (client *Client) AddEventReceiver(msgInstance tdlib.TdMessage, filterFunc EventFilterFunc, channelCapacity int) EventReceiver {
	receiver := EventReceiver{
		...
		Chan:       make(chan tdlib.TdMessage, channelCapacity),
...
func (client *Client) SendAndCatch(jsonQuery interface{}) (tdlib.UpdateMsg, error) {
	...
	case <-time.After(10 * time.Second):
		...
		return tdlib.UpdateMsg{}, errors.New("timeout")
	}
}
